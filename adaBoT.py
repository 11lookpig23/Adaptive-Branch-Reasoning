...
from Prompts.prompthub import DoT_classify,PST_proximity,REL_proximity
import json
import numpy as np
import random
...
## load exmaple pairs
cite_nei_dic = {"cora":cite_neibor_cora,"pubmed":cite_neibor_med}
class Boosterlar:
    def __init__(self,agent,retr):
        ...
        if self.dname == 'cora':
            ...
            self.coexmp = self.dataset.sample_citanode(retr)
            train_ids = [n.idx for n in flatten(self.coexmp.values())]
            self.train_list = list(set(train_ids))
        #if agent.dataname == 'cora':
            test_nodes = agent.node_index_list #self.dataset.sample_testnode(100)
            random.seed(42)
            test_data = random.sample(test_nodes,100)
            rem_nodes = sub_list(test_nodes,test_data)
            test_data.extend(rem_nodes[50:100])
            self.test_data = test_data

        elif self.dname == 'pubmed':
            self.pubexmp = agent.constructExamples(cite_neibor_med)
            self.dataset = dataClassif(agent.data,agent.text,agent.node_index_list,agent.dataname)
            self.train_list = list(set([em[0].idx for em in self.pubexmp.values()]))
            self.test_data = list(self.pubexmp.keys())
        else:
            self.dataset = agent.pstdata
            self.train_list = self.dataset.node_index_list
        #elif agent.dataname == 'pubmed':
    ...
    def Boosting(self,lmodel,start,beta,name,learner_slice,ifsave,load_state,pre_state):
        ...
        for key in learner_slice:#range(self.n_estim):
